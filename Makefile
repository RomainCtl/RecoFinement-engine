...
init: pipenv ## Initialize project
	$(PIPENV) install

serve: ## Serve locally the development build
	$(PYTHON_ENV) run.py
...
# ---------------------
git-branch: ## create new branch [feat|fix|doc|devops] (ex: make git-branch feat="add new feature")
